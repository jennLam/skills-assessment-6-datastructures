...
class LinkedList(object):
    ...
    def print_list(self):
        ...
        # While node is not none, print the node's data
        # Loop until node is reach the end
        while current is not None:
            print current.data
    ...
    def get_node_by_index(self, idx):
        ...
        try:
            # Use a counter as traverse list
            count = 0
            # Set current to head node
            current = self.head

            # Loop list until count is equal the index
            while count != idx:
                # Set current to the next node
                current = current.next
                # Increment the count by 1
                count += 1
            # When the count is equal to the specified index
            # return that node
            return current
