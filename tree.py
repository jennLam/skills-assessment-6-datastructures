...
class Node(object):
    ...
    def get_num_children(self):
        ...
        # Return length of children list
...
class Tree(object):
    ...
    def breadth_first_search(self, data):
        ...
        # Set to_vist to the root node
        to_visit = [self.root]

        # Traverse tree
        while to_visit:
            # Set node to the first item in to_vist list
            node = to_visit.pop(0)

            # If node data is equal to data, return the node
            if node.data == data:
                return node

            # Otherwise, add the node's children to to_visit list
            to_visit.extend(node.children)
